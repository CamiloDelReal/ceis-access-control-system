...
public class PanelTabAccessWorker extends JPanel implements DBActionButton, UpdateTabPanel{
	...
	private JScrollPane sroll = null;
	private JTable table = null;
	private DefaultTableModel tableModel = null;  //  @jve:decl-index=0:visual-constraint="513,124"
	private int oldRow;

	/**
	 * This is the default constructor
	 */
	public PanelTabAccessWorker() {
		super();
		initialize();
	}
	...
	private void initialize() {
		...
		this.add(getSroll(), BorderLayout.CENTER);
	}
	...
	private JPanel getPanelHeader() {
		if (panelHeader == null) {
			...
			labelHeder1.setText("Personal trabajador que ha accedido al centro");
		...
		if (view == null) {
			...
			view.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if(init == null){
						JOptionPane.showMessageDialog(PanelTabAccessWorker.this, "Debe de indicar la fecha inicial de la busqueda", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					Date end = dateEnd.getDate();
					if(end == null){
						JOptionPane.showMessageDialog(PanelTabAccessWorker.this, "Debe de indicar la fecha final de la busqueda", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}					
					...
					try {
						...
						JOptionPane.showMessageDialog(PanelTabAccessWorker.this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
					}
				}
			});
		}
	...
	/**
	 * This method initializes sroll	
	 * 	
	 * @return javax.swing.JScrollPane	
	 */
	private JScrollPane getSroll() {
		if (sroll == null) {
			sroll = new JScrollPane();
			sroll.setViewportView(getTable());
		}
		return sroll;
	}
	...
	private DefaultTableModel getTableModel() {
		if (tableModel == null) {
			tableModel = new DefaultTableModel(new Object[]{"Nombre y apellidos", "CI", "Local", "Fecha"}, 0){
	...
	private void fillTable(Calendar init, Calendar end) throws EmptyDataShowException{
		ControlCollection data = CEIS.getInstance().findAccessWorker(init, end);
		String[] row = new String[4];
		
		if(data == null)
			throw new EmptyDataShowException("No se han registrado entradas al centro en el rango de fechas indicado");
			
		for(int i = 0; i < data.size(); i++){
			row[0] = data.get(i).getWho().toString();
			row[1] = data.get(i).getWho().getCI();
			if(data.get(i) instanceof LocalAccess)
				row[2] = ((LocalAccess)data.get(i)).getLocal().toString();
			else
				row[2] = ((Scheme)data.get(i)).getRulesEntities().toString();
			row[3] = GraphicSupport.getStringEntryDate(data.get(i).getInit(), true);
